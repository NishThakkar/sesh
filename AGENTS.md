...
- **Curated titles win on both generation and display.** `Session.CuratedTitle` marks a title as a real name rather than a raw first prompt — the desktop app generates real session names for both Claude Code Desktop and Cowork sessions, and the user can rename them.
  ...
  - *Exception, set by the provider:* a Cowork scheduled run kept out of a collapse group has `CuratedTitle` cleared. The flag exists to protect a name that identifies the session, and a scheduled run's app title names the recurring task instead — every run of that task carries it, so it identifies nothing. The collapsed representative keeps the flag, since one entry standing for the whole task is exactly what that title describes.
